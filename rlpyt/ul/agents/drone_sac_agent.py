...
from rlpyt.ul.models.rl.drone_rl_models import DroneSacConvEncoder
...
class DroneSacAgent(BaseAgent):
    def __init__(
            self,
            ModelCls=SacModel,  # Module for managing all relevant model for sac agent
            ...
            action_squash=1.,  # config for action gaussian distribution
            pretrain_std=0.75,  # 0.75 gets pretty uniform squashed actions
            load_conv=False,  # just load the pretrained conv encoder
            load_all=False,  # load all sac agent model
            state_dict_filename=None,  # the pretrained param path
            store_latent=False,
            ):
    ...
    def initialize(self, env_spaces, share_memory=False,
        ...
        # self.conv = self.ConvModelCls(**self.conv_kwargs)
        ...
        if share_memory:
            ...
            self.model.share_memory()  # move params(tensors) to a shared memory
        ...
        assert len(env_spaces.action.shape) == 1  # assert action is a vector
        # apply the action as a multi-variable gaussian dist
    ...
    def give_min_itr_learn(self, min_itr_learn):
        self.min_itr_learn = min_itr_learn  # after itrs the start to learn
