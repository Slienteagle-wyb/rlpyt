...
def plot(
    ...
    **kwargs
    ...
    if title is not None:
        font_dict = dict(fontsize=10, fontname="Times New Roman", fontweight="bold")
    ...
    plot_params = {"alpha": kwargs.get("alpha", 1.0), "s": kwargs.get("s", 12), 'linewidths': 0.5}
    ax.scatter(x[:, 0], x[:, 1], c=point_colors, rasterized=True,
               edgecolors='#FAFDFF', **plot_params)
    ...
    if draw_legend:
        ...
        legend_kwargs_ = dict(loc='best', frameon=True, title='Gates', bbox_to_anchor=(0.0, 0.0), fontsize=8,
                              title_fontsize=8, )
        if legend_kwargs is not None:
            legend_kwargs_.update(legend_kwargs)
        legend = ax.legend(handles=legend_handles, **legend_kwargs_)
        legend.get_frame().set_facecolor('#E7E7E5')
